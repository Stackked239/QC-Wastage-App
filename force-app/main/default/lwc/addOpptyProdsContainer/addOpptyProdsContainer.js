...
import { createToastMessage } from 'c/genericMultiToast';
...
import PROD_COLOR from '@salesforce/schema/Product2.SKUID_Color__c';
import PROD_SKU from '@salesforce/schema/Product2.SKUID_SKU__c';
...
const SYNC_TOAST_NAME = 'SYNC_NEEDED';
const TOTAL_GARMENTS_TOAST_NAME = 'GARMENTS_MISMATCH';

export default class AddOpptyProdsContainer extends LightningElement {
    ...
    @wire(getPicklistValues, { recordTypeId: '012000000000000AAA', fieldApiName: PROD_SKU })
    skuCodePicklistVals(value) {
        const { error, data } = value;
        if (data) {
            this.skuCodePicklistValue = data.values;
    ...
    @wire(getPicklistValues, { recordTypeId: '012000000000000AAA', fieldApiName: PROD_COLOR })
    skuColorPicklistVal(value) {
        console.log('199LINE', value);
        const { error, data } = value;
        if (data) {
            this.skuColorPicklistVal = data.values;
        }
    }
    ...
    @wire(serviceProducts, { setup: 'Screen Printing', opptyId: '$recordId' })
    ...
    handleColorCodeChange(event) {
        ...
        this.prodsToInsert[prodKey].Color__c = event.detail.value;
    }

    handleSyncToastMessage() {
        let showToast = false;
        this.prodsToInsert.forEach(prod => {
            if (prod.draftQuantity != null && prod.draftQuantity != undefined) {
                showToast = true;
            }
        });

        if (showToast) {
            let thisToast = this.toastWarnings.filter((toast) => toast.name == SYNC_TOAST_NAME);
            if (thisToast.length == 0) {
                this.toastWarnings.push(createToastMessage(SYNC_TOAST_NAME, 'Don\'t Forget to Sync Your Quantity Changes'));
            }
        } else {
            let toastWarnings = [];
            this.toastWarnings.forEach(toast => {
                if (toast.name == SYNC_TOAST_NAME) { if (!toast.render) { toastWarnings.push(toast); } }
                else { toastWarnings.push(toast); }
            });
            this.toastWarnings = toastWarnings;
        }
    }

    handleDraftQtyChange(event) {
        let prodKey = event.target.dataset.id;
        if (event.detail.value == null || event.detail.value == "") {
            this.prodsToInsert[prodKey].draftQuantity = 0;
        } else {
            this.prodsToInsert[prodKey].draftQuantity = event.detail.value;
        }
        this.prodsToInsert = [...this.prodsToInsert];
        this.handleSyncToastMessage();
    }

    handleSync() {
        let counter = 0;
        let prodsToInsert = [...this.prodsToInsert];
        prodsToInsert.forEach(prod => {
            if (prod?.draftQuantity) {
                this.handleQtyChange({ target: { dataset: { id: counter } }, detail: { value: parseInt(prod.draftQuantity) } });
            }
            prod.draftQuantity = null;
            counter += 1;
        });
        this.prodsToInsert = [...this.prodsToInsert];
        this.handleSyncToastMessage();
        ...
        let that = this;
        setTimeout(() => {
            that.template.querySelectorAll("lightning-input").forEach(field => { field.reportValidity(); });
        }, 200);
    }
    ...
    handleSaveOpptyProds(showToastMsg) {
        ...
        let prodsToInsert = [... this.prodsToInsert];
        prodsToInsert.forEach(prod => { prod.draftQuantity = null; });
        this.prodsToInsert = prodsToInsert;
        this.handleSyncToastMessage();
    ...
    handleCreateOpptyProds() {
        ...
        for (var p = 0; p < this.prodsToInsert.length; p++) {
            ...
            tempOPObj.SKUID_Color__c = this.prodsToInsert[p].Color__c;
    ...
    reCalculateTotals() {
        ...
        if (this.qtytotal != this.garments) {
            let thisToast = this.toastWarnings.filter((toast) => toast.name == TOTAL_GARMENTS_TOAST_NAME);
            if (thisToast.length == 0) {
                this.toastWarnings.push(createToastMessage(TOTAL_GARMENTS_TOAST_NAME, 'Total Garments does not match Total Quantity'));
            }
        } else {
            let toastWarnings = [];
            this.toastWarnings.forEach(toast => {
                if (toast.name == TOTAL_GARMENTS_TOAST_NAME) { if (!toast.render) { toastWarnings.push(toast); } }
                else { toastWarnings.push(toast); }
            });
            this.toastWarnings = toastWarnings;
        }
    ...
    handleToastChanges(event) { this.toastWarnings = event.detail; console.log('4'); }
}